...
It is a mathematical olympiad for sixth and seventh grades that is held each February.
...
(P.S. "ФЕВРАЛЬ" in Russian means "FEBRUARY". The olympiad is usually held in February.)
I was asked to find all possible solutions so that the olympiad examiners could check children's works faster.

## At first, let's start with an idea of the program!
...
On the contrary, this way is faster than manipulation with sequences for such a number of elements.
...
First, we create a data class for the permutations and a simple fake constructor from a list for it:
```kotlin
...
Then, let's check that the permutation is correct:
```kotlin
...
At second, we have just to filter out the wrong permutations:
```kotlin
val февральPermutations: Sequence<ФЕВРАЛЬ> =
    digits.permutations(7u)
        .map { ФЕВРАЛЬ(it) }
...
Now we have all the solutions in `февральPermutations` variable. Let's print them in three different formats:
```kotlin
fun main() {
...
If you are not familiar with Kotlin and/or Gradle,
see [this guide](/docs/kotlin-gradle-project-setup) to set up your environment.